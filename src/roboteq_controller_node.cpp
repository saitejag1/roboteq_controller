...
void RoboteqDriver::cmdSetup(){
	...
	right_enccmd << "^EPPR 1 " << 4096 << "\r";
	left_enccmd << "^EPPR 2 " << 4096 << "\r";
...
void RoboteqDriver::cmdVelCallback(const geometry_msgs::msg::Twist &msg){
	// wheel speed (m/s)
	float right_speed = msg.linear.x + 0.8786 * msg.angular.z / 2.0;
	float left_speed  = msg.linear.x - 0.8786 * msg.angular.z / 2.0;
	...
	if (!closed_loop_){
		...
		// motor speed (rpm)
		int32_t right_rpm = right_speed *60.0 / 0.628;
		int32_t left_rpm  = left_speed  *60.0 / 0.628;
